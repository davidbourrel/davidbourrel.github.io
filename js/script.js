...
const MyAPP = function () {
  ...
  document
    .querySelectorAll('.reveal-first-effect')
    .forEach((elements) => observerLEFT.observe(elements));
  const observerRIGHT = new IntersectionObserver(callbackBOTTOM, options);
  document
    .querySelectorAll('.reveal-second-effect')
    .forEach((elements) => observerRIGHT.observe(elements));
  /* //////////////// QUIZ \\\\\\\\\\\\\\\\\\\\\ */
  ...
  form.addEventListener('submit', function (e) {
    ...
    const checks = document.querySelectorAll('.question-icon.fa-check');
    const crosses = document.querySelectorAll('.question-icon.fa-times');
    ...
    const radioCustomLabel = document.querySelectorAll('.radio-custom-label');
    radioCustoms.forEach((radioCustom, index) => {
      if (radioCustom.value.includes('right')) {
        radioCustomLabel[index].style.cssText =
          'color:#046b26 font-weight: bold';
      }
    });
    ...
    if (score <= 29) {
      ...
      quizComment.textContent =
        'Super ! Tu commence à bien comprendre le cours!';
    ...
    resetButton.addEventListener('click', function () {
      ...
      radioCustoms.forEach((radioCustom, index) => {
        if (radioCustom.value.includes('right')) {
          radioCustomLabel[index].style.cssText =
            'color: rgba(20, 20, 20, 0.801) font-weight: inherit';
        }
      });
